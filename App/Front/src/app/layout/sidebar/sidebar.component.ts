...
export class SidebarComponent {
  ...
  { label: 'Empleados',   route: '/empleados',       icon: 'fa-users-gear',     systemRoles: ['ADMIN'] },

  { label: 'Mi Equipo', route: '/mi-equipo', icon: 'fa-sitemap', systemRoles: ['ADMIN', 'USER'], companyRoles: ['MANDO'] },
  ...
  return this.menuItems.filter(item => {
    if (item.systemRoles && (!systemRole || !item.systemRoles.includes(systemRole))) return false;
    if (item.companyRoles && (!companyRole || !item.companyRoles.includes(companyRole))) return false;
    return true;
  });
}
